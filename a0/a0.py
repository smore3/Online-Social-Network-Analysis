...
def print_num_friends(users):
    ...
    #users.sort()
    for i in users:
        print(str(i['screen_name']), len(i['friends']))
    pass
...
def friend_overlap(users):
    ...
    for n in range(len(users)):
        candidate1=users[n]['screen_name']
        for m in range(n+1,len(users)):
            candidate2=users[m]['screen_name']

            common_friends=set(users[n]['friends']).intersection(users[m]['friends'])
            t = (candidate1,candidate2,len(common_friends))
            common_friends_list.append(t)

    common_friends_list_sorted=sorted(common_friends_list,key=lambda x:x[2], reverse=True)
    return common_friends_list_sorted
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    ###TODO
    #hillary_list = [n['friends'] for n in users if n['screen_name']=='HillaryClinton']
    #trump_list = [n['friends'] for n in users if n['screen_name']=='realDonaldTrump']
    for u in users:
        if u['screen_name']=='HillaryClinton':
            hillary_list=u['friends']
        if u['screen_name']=='realDonaldTrump':
            trump_list=u['friends']
    common = set(hillary_list).intersection(trump_list)

    user_detail = robust_request(twitter, 'users/lookup', {'user_id': common})
    for u in user_detail:
        common_name = u['screen_name']
...
def create_graph(users, friend_counts):
    ...
    for u in users:
        ...
        for v in u['friends']:
            if friend_counts[v] > 1:
                graph.add_node(v)

    for u in users:
        for v in u['friends']:
            if (friend_counts[v] > 1):
                graph.add_edge(u['screen_name'], v)
...
def draw_network(graph, users, filename):
    ...
    #labels = {n['screen_name']: n['screen_name'] for n in users}
    #print(labels)
    nx.draw_networkx(graph, with_labels=False)
    nx.draw_networkx(graph, pos=nx.spring_layout(graph), labels=labels)
...
def main():
    ...
    add_all_friends(twitter, users)
    #print(str([u['friends'] for u in users]))
